...
| Component | Max | Logic |
|-----------|-----|-------|
| Consecutive candles | 10 | 4+ candles same direction (close > open for UP): +10. 3: +7. 2: +3. |
| Higher highs/higher lows | 10 | Last 3 candles show HH+HL (uptrend) or LL+LH (downtrend): +10. 2 of 3: +5. |
...
| Component | Max | Logic |
|-----------|-----|-------|
| Volume trend | 8 | Volume increasing for 2+ candles: +8. Flat: +3. Decreasing: 0. |
| Volume vs average | 7 | Volume > 2x vol_sma_20: +7. > 1.5x: +4. > 1x: +2. |

Direction determined by which side (UP vs DOWN) accumulates more points.
...
| RSI divergence | -10 | Price making higher high but RSI lower (UP), or lower low but RSI higher (DOWN), over last 3 candles: -10. |
...
**Normalization:** `confidence = min(100, max(0, raw_score * 100 / 180))`
...
out["adx"] = adx_df.iloc[:, 0]  # ADX line
```
...
**Comparison:**
1. v1 predictor → WR, bets, PF, per-asset
2. v2 predictor → same metrics
3. Side-by-side on identical data
