# K15 Rich Interactive Dashboard — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an interactive terminal dashboard (`k15.py`) with `rich` live-updating panels, command prompt, and integrated bot control for the Kalshi V3 predictor.

**Architecture:** Single Python file using `rich.live.Live` for flicker-free display with two panels (status+predictions left, activity log right) and a command prompt at the bottom. Main thread handles input, daemon thread runs the eval loop. Commands start with `/`.

**Tech Stack:** Python 3, rich, threading

**Spec:** `docs/superpowers/specs/2026-03-25-k15-rich-dashboard-design.md`

---

### Task 1: Install `rich` and verify
...
```bash
cd /Users/jordan/Documents/Dev/algotrade && ./venv/bin/pip install rich
```

- [ ] **Step 2: Verify**

```bash
cd /Users/jordan/Documents/Dev/algotrade && ./venv/bin/python -c "from rich.live import Live; from rich.layout import Layout; from rich.table import Table; from rich.panel import Panel; print('rich OK')"
```

- [ ] **Step 3: Commit**

No commit needed — `rich` is a runtime dependency, not code.

---

### Task 2: Create `k15.py` with layout and static rendering

**Files:**
- Create: `k15.py`

- [ ] **Step 1: Create `k15.py` with the core layout**
...
```bash
chmod +x /Users/jordan/Documents/Dev/algotrade/k15.py
```

- [ ] **Step 3: Run to verify layout renders**

```bash
cd /Users/jordan/Documents/Dev/algotrade && ./venv/bin/python k15.py
```

Type `/` to see commands, then `/quit` to exit. Verify:
- Two panels render (left: status, right: activity log)
- "No predictions yet" shows in left panel
- "No activity yet" shows in right panel
- Command prompt works

- [ ] **Step 4: Test `/dry-run` command**

Start `k15.py`, type `/dry-run`. Verify:
- "Starting V3 dry-run..." appears
- Activity log shows "Started DRY-RUN V3"
- After ~15 seconds, predictions appear in left panel with probabilities
- Dashboard re-renders on each command entry
...
```bash
cd /Users/jordan/Documents/Dev/algotrade && git add k15.py && git commit -m "feat: add K15 rich interactive dashboard with /commands

Two-panel layout: status+predictions (left), activity log (right).
Commands: /dry-run, /live, /stop, /status, /balance, /closeall, /quit.
/live and /closeall require YES confirmation.
...
### Task 3: Add auto-refresh with Live display

**Files:**
- Modify: `k15.py`

The current implementation re-renders on each command entry. This task adds continuous auto-refresh using a background thread while still accepting input.

- [ ] **Step 1: Restructure main loop for continuous refresh**

Replace the `main()` function's loop with a threaded approach:
- Background thread: refreshes the dashboard display every 2 seconds
- Main thread: blocks on `input()` waiting for commands

The trick: `rich.live.Live` and `input()` conflict on the same terminal. Instead, use a simpler approach — print the dashboard, clear screen, re-print. Or use `rich.console.Console.clear()` before each re-render.

A cleaner approach: use the "render on command + auto-refresh timer" pattern:
...
This gives auto-refresh every 2 seconds AND responsive command input.

- [ ] **Step 2: Test auto-refresh**

Start k15.py, type `/dry-run`. Verify:
- Dashboard refreshes automatically every 2 seconds
- Predictions update as eval cycles fire
- Typing commands still works during refresh
- No flickering

- [ ] **Step 3: Commit**

```bash
cd /Users/jordan/Documents/Dev/algotrade && git add k15.py && git commit -m "feat: add auto-refresh to K15 dashboard (2-second interval)
...
### Task 4: Add `--simple` fallback to `dashboard.py`
...
At the start of `dashboard.py`'s `main()`, add a check:

```python
# If not a TTY (piped/redirected) or --simple flag, use plain text output
if not sys.stdout.isatty() or args.simple:
...
This ensures MCP's `algotrade_start` (which pipes to a log file) continues to work with plain text.

- [ ] **Step 2: Commit**

```bash
cd /Users/jordan/Documents/Dev/algotrade && git add dashboard.py && git commit -m "feat: add --simple flag and TTY detection to dashboard.py
